...
// Variáveis para rastreamento de testes
$testesPassados = 0;
$testesFalhados = 0;
...
function mostrarResultado(string $nomeTeste, bool $resultado): void
{
    ...
    if ($resultado) {
        echo "<p style='color: green; font-weight: bold;'>✓ PASSOU: $nomeTeste</p>";
        $testesPassados++;
    } else {
        echo "<p style='color: red; font-weight: bold;'>✗ FALHOU: $nomeTeste</p>";
        $testesFalhados++;
    }
}
...
echo "<span style='color: green;'>✓ Testes passados: $testesPassados</span> | ";
echo "<span style='color: red;'>✗ Testes falhados: $testesFalhados</span>";
echo "</p>";

$totalTestes = $testesPassados + $testesFalhados;
$percentualPassagem = ($totalTestes > 0) ? round(($testesPassados / $totalTestes) * 100, 2) : 0;

echo "<p style='font-size: 16px;'>";
echo "Percentual de sucesso: <strong>$percentualPassagem%</strong>";
echo "</p>";

if ($testesFalhados === 0) {
    echo "<p style='color: green; font-size: 20px; font-weight: bold;'>🎉 TODOS OS TESTES PASSARAM!</p>";
}
